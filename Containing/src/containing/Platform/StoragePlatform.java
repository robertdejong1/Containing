...
public class StoragePlatform extends Platform {
    ...
    private void createAgvSpots()
    {
        ...
        for(int i = 0; i < StorageStrip.MAX_AGV_SPOTS*getStripAmount(); i++) 
        {
            ...
            if(i % 2 == 0)
            {
                agvSpotPosition = new Vector3f(getPosition().x + AGV_OFFSET - 0.75f, getPosition().y, z + (subcount*offset) + (AGV.width*subcount)*Settings.METER + 0.05f*subcount + 0.15f + (AGV.width*Settings.METER) / 2f);
            }
            else
            {
